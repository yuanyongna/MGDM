...
from ..encoders import (PaiNNEncoder, SchNetEncoder,
                        SchNetEncoder_protein, get_edge_encoder)
...
class mgdm_adapter_pocket(nn.Module):

    def __init__(self, config):
        super(mgdm_adapter_pocket, self).__init__()
        self.config = config
        ...
        # self.hidden_dim += config.protein_hidden_dim
        self.encoder_type = getattr(config, 'encoder_type', 'egnn')
        ### noncovalent encoder
        if self.encoder_type == 'painn':
            self.encoder_global = PaiNNEncoder(
                n_layers=config.num_convs,
                feats_input_dim=self.hidden_dim,  # pocket_atom is already hidden_dim after encoders/attention
        ...
        ### covalent encoder
        if self.encoder_type == 'painn':
            self.encoder_local = PaiNNEncoder(
    ...
    def net(self, ligand_atom_type, ligand_pos, ligand_bond_index, ligand_bond_type, ligand_batch,
        ...
        # PaiNN outputs are pocket-length; trim to ligand nodes for loss alignment
        if self.encoder_type == 'painn':
            node_attr_global = node_attr_global[:N]
            pos_attr_global = pos_attr_global[:N]
            node_attr_local = node_attr_local[:N]
            pos_attr_local = pos_attr_local[:N]

        # Debug: guard against NaNs/Infs from encoders
        if torch.isnan(pos_attr_global).any() or torch.isinf(pos_attr_global).any():
            print("[MGDM] NaN/Inf in pos_attr_global; zeroing")
            pos_attr_global = torch.nan_to_num(pos_attr_global, nan=0.0, posinf=0.0, neginf=0.0)
        if torch.isnan(pos_attr_local).any() or torch.isinf(pos_attr_local).any():
            print("[MGDM] NaN/Inf in pos_attr_local; zeroing")
            pos_attr_local = torch.nan_to_num(pos_attr_local, nan=0.0, posinf=0.0, neginf=0.0)
        if torch.isnan(node_attr_global).any() or torch.isinf(node_attr_global).any():
            print("[MGDM] NaN/Inf in node_attr_global; zeroing")
            node_attr_global = torch.nan_to_num(node_attr_global, nan=0.0, posinf=0.0, neginf=0.0)
        if torch.isnan(node_attr_local).any() or torch.isinf(node_attr_local).any():
            print("[MGDM] NaN/Inf in node_attr_local; zeroing")
            node_attr_local = torch.nan_to_num(node_attr_local, nan=0.0, posinf=0.0, neginf=0.0)
    ...
    def forward(self, batch, context=None, return_unreduced_loss=False, return_unreduced_edge_loss=False,
        ...
        # score matching
        # Clamp edge lengths to avoid division by zero in eq_transform
        edge_length = torch.clamp(edge_length, min=1e-6)
        edge_length_local = torch.clamp(edge_length[local_edge_mask], min=1e-6)
        ...
        target_pos_local = eq_transform(d_target[local_edge_mask], ligand_pos_perturbed, edge_index[:, local_edge_mask],
                                        edge_length_local)
        # Clamp/cleanup to avoid exploding values
        pos_eq_global = torch.nan_to_num(pos_eq_global, nan=0.0, posinf=0.0, neginf=0.0).clamp(min=-5.0, max=5.0)
        pos_eq_local = torch.nan_to_num(pos_eq_local, nan=0.0, posinf=0.0, neginf=0.0).clamp(min=-5.0, max=5.0)
        target_pos_global = torch.nan_to_num(target_pos_global, nan=0.0, posinf=0.0, neginf=0.0).clamp(min=-5.0, max=5.0)
        target_pos_local = torch.nan_to_num(target_pos_local, nan=0.0, posinf=0.0, neginf=0.0).clamp(min=-5.0, max=5.0)
        loss_pos = F.mse_loss(pos_eq_global + pos_eq_local, target_pos_global + target_pos_local, reduction='none')
        loss_pos = torch.nan_to_num(loss_pos, nan=0.0, posinf=0.0, neginf=0.0)
        ...
        loss_node = torch.nan_to_num(loss_node, nan=0.0, posinf=0.0, neginf=0.0)
        loss_node = 0.001 * torch.sum(loss_node, dim=-1, keepdim=True)  # small weight to gradually restore node branch
