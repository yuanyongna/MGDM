...
def train(it):
        ...
        if args.use_mixed_precision:
            with tqdm(total=len(train_loader), desc='Training') as pbar:
                for i, batch in enumerate(train_loader):
                    ...
                    if 'adapter' in config.model.network:
                    ...
                    if 'adapter' in config.model.network:
                        ...
                        for p in model.parameters():
                            if p.grad is not None and not torch.isfinite(p.grad).all():
                                ...
                                p.grad = None
                        if has_nan_grad:
                            print(f"[Train] skip step at epoch {it}, batch {i} due to NaN grad")
                            optimizer.zero_grad()
                            continue
        ...
        with open(log_txt_path, 'a') as f:
            f.write('%s | [Train] Epoch %05d | Loss %.2f | Loss(Global) %.2f | Loss(Local) %.2f | Loss(node_global) %.2f | Loss(node_local) %.2f | Loss(vae_KL) %.2f |Grad %.2f | LR %.6f\n' % (
                datetime.now().isoformat(), it, avg_loss, avg_loss_global, avg_loss_local, avg_loss_node_global, avg_loss_node_local, loss_vae_KL, orig_grad_norm, optimizer.param_groups[0]['lr']))
...
if __name__ == '__main__':
    ...
    log_txt_path = os.path.join(log_dir, 'logs.txt')
